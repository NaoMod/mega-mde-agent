...
![Diversirty Analysis(54 tools)](images/bar_comparison.png)

![lexical_diversity(54 tools)](images/lexical_diversity.png)

![Diversirty Analysis(16 UML related tools)](images/uml_bar_comparison.png)

![lexical_diversity(16 UML related tools)](images/uml_lexical_diversity.png)

![Diversirty Analysis(12 Table related tools)](images/table_bar_comparison.png)

![lexical_diversity(12 Table related tools)](images/table_lexical_diversity.png)
...
**UML Tools:**
The LLM’s augmentation of UML-related instructions reveals a nuanced pattern in topical diversity. For single-tool scenarios, the generated instructions become even more topically cohesive than the seeds (distance: 0.889 → 0.861). This suggests that the LLM is highly effective at producing new instructions that remain tightly anchored to the core transformation patterns present in the original UML seeds, reinforcing domain fidelity and minimizing drift into unrelated topics. For multi-tool workflows, the distance increases slightly (0.747 → 0.827), but the values remain close—indicating that even as the LLM combines multiple UML tools, it maintains strong topic fidelity. Rather than diverging into unrelated areas, the generated multi-tool instructions continue to cluster around the core transformation scenarios, preserving the semantic focus of the seed examples. This subtle increase reflects the added complexity of coordinating multiple operations, but the overall dataset remains cohesive and well-aligned with the intended UML domain.

**Table Tools:**
For Table-related tools, the LLM’s augmentation process consistently reduces the topical distance for both single-tool and multi-tool instructions (single-tool: 0.966 → 0.854, multi-tool: 0.830 → 0.782). This pattern indicates that the generated Table instructions cluster more closely around the original seed patterns, emphasizing semantic fidelity and operational consistency. The LLM appears to prioritize the preservation of core table manipulation concepts, generating instructions that reinforce established transformation types rather than venturing into unrelated or highly novel territory. This convergence is particularly valuable for evaluation settings where maintaining a clear, interpretable relationship to the original task domain is essential. By producing a dataset with reduced topical spread, the LLM ensures that agents are tested on a comprehensive yet coherent set of table transformation scenarios, minimizing noise and maximizing the relevance of each generated instruction to real-world table transformation tasks.
...
**UML Tools:**
For single-tool, the generated instructions are more semantically consistent (dispersion: 0.412 → 0.390). For multi-tool, the generated instructions are more semantically diverse (dispersion: 0.291 → 0.352), showing that augmentation increases the diversity of meanings for multi-step UML instructions.

**Table Tools:**
For both single-tool and multi-tool, the generated instructions are more semantically consistent than the seeds (single-tool: 0.473 → 0.385, multi-tool: 0.354 → 0.318), indicating that the augmentation process makes Table instructions more uniform in intent and meaning.
...
**UML Tools:**
Isocontour radius increases for both single-tool (0.014 → 0.015) and multi-tool (0.012 → 0.014), indicating that the generated instructions are more spread out across embedding dimensions, especially for multi-tool.

**Table Tools:**
Isocontour radius is stable for single-tool (0.015 → 0.015) and increases for multi-tool (0.013 → 0.014), showing a slight increase in multi-dimensional diversity for Table multi-tool instructions after augmentation.
...
**UML & Table Tools:**
For both UML and Table tool subsets, the augmentation process leads to a substantial increase in vocabulary size for single-tool and multi-tool instructions. However, compared to the full toolset, the overall diversity is lower because these analyses focus on a smaller subset of tools, naturally limiting the range of technical terms and expressions that can appear. This growth still demonstrates the LLM’s ability to express similar operations in multiple ways and to introduce a variety of technical expressions, synonyms, and context-specific phrases relevant to each domain. The resulting datasets maintain strong domain fidelity and conceptual coverage, but the lexical diversity is naturally constrained by the narrower operational focus of the selected tool subsets.
...
**UML & Table Tools:**
For both UML and Table tool subsets, the augmentation process leads to an increase in phrase-level diversity, as reflected by the unique three-word sequences (3-grams) across both single-tool and multi-tool instructions. Rather than simply rephrasing instructions with minor edits, the LLM actively generates a variety of sentence structures, action sequences, and descriptive patterns. This results in datasets where similar transformation intents are expressed through a multitude of linguistic forms, challenging agents to recognize and interpret instructions that differ in surface realization but share underlying meaning.
